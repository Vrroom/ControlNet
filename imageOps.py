...
def make_image_grid (images, row_major=True, gutter=True, img_type='RGB'):
    """
    ...
    if isinstance(images[0], list) :
        rows = [make_image_grid(row, row_major=True, gutter=gutter, img_type=img_type) for row in images]
        return make_image_grid(rows, row_major=False, gutter=gutter, img_type=img_type)
    else :
        if row_major :
...
def aspectRatioPreservingResize (arr, smaller_dim, resample=Image.LANCZOS) :
    ...
    h, w = int(h), int(w)
    resized = pil_img.resize((h, w), resample=resample)
    np_arr = np.array(resized).astype(arr.dtype)
    if arr.dtype in [float, np.float32, np.float64] :
        np_arr /= 255.0
    return np_arr

def aspectRatioPreservingResizePIL (pil_img, smaller_dim, resample=Image.LANCZOS) :
    """ utility for resizing image, ensuring that smaller dimension matches """
    h, w = pil_img.size
    if h < w :
        h, w = smaller_dim, smaller_dim * w / h
